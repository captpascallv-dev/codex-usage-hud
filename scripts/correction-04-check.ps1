...
$evidenceRoot = Join-Path $projectRoot ".artifacts\correction-04\check\$timestamp"
$evidencePath = Join-Path $evidenceRoot 'correction-04-check.txt'
...
$output = @(& $dotnet run --project $testProject -c Release --no-build --no-restore -- --correction-04-check 2>&1)
...
Write-Output "CORRECTION_04_EVIDENCE path=$relativeEvidence"
exit 0
